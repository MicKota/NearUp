...
export default function UserProfile() {
  ...
  const fromEventId = (params as any)?.fromEventId;
  ...
  useEffect(() => {
    async function fetchProfileAndEvents() {
      if (userId) {
        ...
        const joinedQuery = query(collection(db, 'events'), where('participants', 'array-contains', userId));
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ alignItems: 'center', paddingBottom: 40, paddingTop: 20 }}>
      ...
      <Text style={styles.label}>Wydarzenia, do których dołączył użytkownik:</Text>
      {joinedEvents.length === 0 ? (
        <Text style={{ color: '#888', marginBottom: 10 }}>Brak dołączonych wydarzeń.</Text>
      ) : (
